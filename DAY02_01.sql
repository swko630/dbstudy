...
ALTER TABLE CUST_T DROP COLUMN CUST_AGE;

-- 4. CUSTOMER_T의 연락처(CUST_PHONE) 칼럼명을 CUST_TEL로 수정하시오.
ALTER TABLE CUST_T RENAMED COLUMN CUST_PHONE TO CUST_TEL;

-- 5. CUSTOMER_T에 등급(GRADE) 칼럼을 추가하시오. 'VIP','GOLD','SLIVER','BRONZE'중 하나의 값을 가지도록 설정하시오.
ALTER TABLE CUST_T ADD GRADE VARCHAR2(6 BYTE) CHECK (GRADE = 'VIP' OR 'GOLD' OR 'SLIVER' OR 'BRONZE');
